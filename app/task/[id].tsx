...
export default function TaskDetailScreen() {
  ...
  const { tasks, projects, updateTask, deleteTask, toggleTaskDone } = useApp();
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[styles.titleRow, { borderLeftColor: pColor, borderLeftWidth: task.priority !== "none" ? 4 : 0 }]}>
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderRadius: colors.radius }]}>
          ...
          {linkedProject && (
            <View style={styles.metaRow}>
              <Ionicons name="folder-outline" size={16} color={linkedProject.color ?? colors.primary} />
              <Text style={[styles.metaLabel, { color: colors.mutedForeground }]}>Project</Text>
              <Text style={[styles.metaValue, { color: linkedProject.color ?? colors.primary }]}>
                {linkedProject.name}
              </Text>
            </View>
          )}
          {task.isRepeating && (
            <View style={styles.metaRow}>
              ...
              <Text style={[styles.metaValue, { color: colors.primary }]}>{task.repeatType ?? ""}</Text>
            </View>
          )}
