...
function LockScreen({ onUnlock }: { onUnlock: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.lockSub, { color: colors.mutedForeground }]}>Enter your 4-digit passcode</Text>
      <View style={styles.dotsRow}>
        {DOTS.map((d) => (
          <View
            ...
            style={[
              styles.dot,
              { backgroundColor: input.length >= d ? (error ? colors.destructive : colors.primary) : colors.border },
            ]}
          />
        ))}
      </View>
...
function PrivateContent() {
  ...
  const delta =
    moneyRows.length >= 2
      ? moneyRows[moneyRows.length - 1].totalSum - moneyRows[moneyRows.length - 2].totalSum
      : 0;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.tabRow}>
        {(["diary", "money"] as const).map((t) => (
          <TouchableOpacity
            key={t}
            style={[
              styles.tabBtn,
              { backgroundColor: tab === t ? colors.primary : colors.secondary, borderRadius: colors.radius },
            ]}
            onPress={() => setTab(t)}
          >
            <Ionicons
              name={t === "diary" ? "book-outline" : "cash-outline"}
              size={16}
      ...
      {tab === "diary" && (
        ...
        >
          ...
          >
            ...
            <Text style={[styles.addBtnText, { color: colors.primaryForeground }]}>New Diary Entry</Text>
          </TouchableOpacity>

          {diaryEntries.length === 0 && (
            <View style={styles.empty}>
              <Ionicons name="book-outline" size={48} color={colors.mutedForeground} />
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No diary entries yet</Text>
            </View>
          )}

          {diaryEntries.map((entry, idx) => (
            <Animated.View key={entry.id} entering={FadeInDown.delay(idx * 30).duration(300)}>
              <TouchableOpacity
                style={[
                  styles.diaryCard,
      ...
      {tab === "money" && (
        ...
        >
          <View style={styles.moneyCards}>
            <View
              style={[styles.totalCard, { backgroundColor: "#f59e0b22", borderRadius: colors.radius * 1.5, borderColor: "#f59e0b44" }]}
            >
              <Text style={[styles.totalLabel, { color: "#f59e0b" }]}>Total Money</Text>
              <Text style={[styles.totalAmount, { color: "#f59e0b" }]}>{totalMoney.toLocaleString()} EGP</Text>
            </View>
            <View
              style={[
                styles.deltaCard,
            ...
            >
              <Text style={[styles.totalLabel, { color: delta >= 0 ? colors.success : colors.destructive }]}>
          ...
          <TouchableOpacity
            style={[styles.addBtn, { backgroundColor: colors.primary, borderRadius: colors.radius, marginBottom: 12 }]}
            onPress={() => router.push("/money/add")}
          >
            <Ionicons name="add" size={20} color={colors.primaryForeground} />
            <Text style={[styles.addBtnText, { color: colors.primaryForeground }]}>Add Money Entry</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.addBtn, { backgroundColor: colors.secondary, borderRadius: colors.radius, marginBottom: 16 }]}
            onPress={() => router.push("/money/table")}
          >
            <Ionicons name="grid-outline" size={20} color={colors.foreground} />
            <Text style={[styles.addBtnText, { color: colors.foreground }]}>View Table</Text>
          </TouchableOpacity>

          {moneyRows.slice().reverse().map((row, idx) => (
            <Animated.View key={row.id} entering={FadeInDown.delay(idx * 30).duration(300)}>
              <TouchableOpacity
                style={[styles.moneyRowCard, { backgroundColor: colors.card, borderRadius: colors.radius, borderColor: colors.border }]}
                onPress={() => router.push("/money/table")}
              >
                <View style={styles.moneyRowHeader}>
                  <Text style={[styles.moneyDate, { color: colors.foreground }]}>{row.day} {row.date}</Text>
                  <Text style={[styles.moneyTotal, { color: colors.primary }]}>{row.totalSum.toLocaleString()} EGP</Text>
                </View>
                {activeCategories.map((cat) => {
                  ...
                  return (
                    <View key={cat.id} style={styles.catRow}>
                      ...
                      <Text style={[styles.catName, { color: colors.mutedForeground }]}>{cat.nameAr}</Text>
                      <Text style={[styles.catAmt, { color: colors.foreground }]}>{val.toLocaleString()}</Text>
                    </View>
                  );
                })}
              </TouchableOpacity>
          ...
          {moneyRows.length === 0 && (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No money entries yet</Text>
            </View>
          )}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  lockContainer: { flex: 1, alignItems: "center", paddingHorizontal: 40, gap: 16 },
  ...
  addBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, height: 46, marginBottom: 12 },
  ...
  moneyRowCard: { padding: 14, marginBottom: 10, borderWidth: 1 },
